...
const MAY_31 = new Date(2026, 4, 31);
...
describe('Datepicker', () => {
  ...
  it('renders a visible label and associates it with the input', () => {
    render(<Datepicker label="Appointment date" value={null} onChange={() => {}} />);
  ...
  it('wires aria-label to the input when label prop is absent', () => {
    render(
      <Datepicker aria-label="Appointment date" value={null} onChange={() => {}} />,
    );
    expect(screen.getByRole('textbox', { name: 'Appointment date' })).toBeInTheDocument();
  });
  ...
  it('dialog has role="dialog", aria-modal="true", and aria-labelledby pointing at the heading', async () => {
    const user = userEvent.setup();
    render(<Datepicker label="Date" value={MAY_15} onChange={() => {}} locale="en-US" />);
    await user.click(screen.getByRole('button', { name: /open date picker/i }));
  ...
  it('clicking a day calls onChange with the selected date and closes the dialog', async () => {
    ...
    render(<Datepicker label="Date" value={null} onChange={handleChange} locale="en-US" />);
    await user.click(screen.getByRole('button', { name: /open date picker/i }));
    const cells = screen.getAllByRole('button', { name: /15/ });
    const dayBtn = cells.find((el) => el.classList.contains('artui-dp-day-btn'));
    if (dayBtn) {
  ...
  it('disabled days cannot be selected', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /open date picker/i }));
    const disabled = document.querySelector<HTMLElement>('button[aria-disabled="true"]');
    if (disabled) {
  ...
  it('Enter on a focused day selects it, calls onChange, and closes dialog', async () => {
    const user = userEvent.setup();
    const handleChange = vi.fn();
    render(<Datepicker label="Date" value={MAY_15} onChange={handleChange} locale="en-US" />);
    await user.click(screen.getByRole('button', { name: /open date picker/i }));
  ...
  it('external error prop is displayed and wired via aria-describedby', () => {
    render(
      <Datepicker label="Date" value={null} onChange={() => {}} error="Date is required" />,
    );
    const input = screen.getByRole('textbox');
    const errorId = input.getAttribute('aria-describedby');
    const errorEl = document.getElementById(errorId!);
  ...
  it('days before min are disabled', async () => {
    const user = userEvent.setup();
    render(
      <Datepicker label="Date" value={MAY_15} onChange={() => {}} min={MAY_15} locale="en-US" />,
    );
    await user.click(screen.getByRole('button', { name: /open date picker/i }));
  ...
  it('days after max are disabled', async () => {
    const user = userEvent.setup();
    render(
      <Datepicker label="Date" value={MAY_15} onChange={() => {}} max={MAY_15} locale="en-US" />,
    );
    await user.click(screen.getByRole('button', { name: /open date picker/i }));
  ...
  // -------------------------------------------------------------------------
  // Runtime overlay guards
  // -------------------------------------------------------------------------

  it('fires dev overlay when min > max', () => {
    render(
      <Datepicker
        label="Date"
        value={null}
    ...
    expect(msg).toContain('[artui]');
    expect(msg).toContain('Datepicker');
    expect(msg).toContain('WCAG 1.3.5');
  });

  it('fires dev overlay when value is before min', () => {
    render(
      <Datepicker
        label="Date"
        value={MAY_1}
  ...
  it('fires dev overlay when value is after max', () => {
    render(
      <Datepicker
        label="Date"
        value={JUNE_1}
  ...
  it('fires dev overlay when isDateDisabled returns true for current value', () => {
    render(
      <Datepicker
        label="Date"
        value={MAY_15}
        onChange={() => {}}
        isDateDisabled={(d) => d.getDay() === 5} // May 15 2026 is a Friday
      />,
    );
    expect(errorSpy).toHaveBeenCalled();
    const msg = String(errorSpy.mock.calls[0]?.[0] ?? '');
    expect(msg).toContain('disabled by isDateDisabled');
  });

  it('fires dev overlay for invalid BCP-47 locale', () => {
    render(
      <Datepicker label="Date" value={null} onChange={() => {}} locale="not-valid-locale-xyz" />,
    );
    expect(errorSpy).toHaveBeenCalled();
    const msg = String(errorSpy.mock.calls[0]?.[0] ?? '');
