...
- Files and directories: kebab-case everywhere, including components (`focus-trap.ts`, `registry/components/image/image.tsx`). No PascalCase filenames.
- Exported component identifiers: PascalCase (`Image`, `FocusTrap`). The component _file_ stays kebab-case.
...
- Abbreviations only when universally known (`id`, `url`, `api`, `auth`). Acronyms as words: `userId`, not `userID`.
