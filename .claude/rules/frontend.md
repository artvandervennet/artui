...
This project uses **Tailwind v4** (in `apps/docs`) and **vanilla CSS / authored styles** in `registry/styles`. Registry components must not introduce new CSS frameworks — they ship as source to consumers and must stay framework-agnostic at the styling layer.

| Category | Used here |
|---|---|
| CSS | Tailwind v4 (docs only), plain CSS for registry components |
| Primitives | None bundled — registry components are self-contained, accessibility-first primitives |
| Animation | CSS transitions only (registry must respect `prefers-reduced-motion`) |
| Icons | None bundled in registry; docs may use any |
