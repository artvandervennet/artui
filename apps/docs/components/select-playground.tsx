...
export function SelectPlayground() {
  ...
  function buildCode(): string {
    ...
    if (isMulti) {
      ...
      if (hasClearAll)
        controlAttrs.push("showClearAll", 'clearAllLabel="Clear all"');
    ...
    if (isGrouped) {
      for (const [group, opts] of Object.entries(GROUPED_OPTIONS)) {
        ...
        for (const opt of opts) {
          lines.push(
            `${indent}  <Select.Option value="${opt.value}">${opt.label}</Select.Option>`,
          );
        }
        ...
        <>
          ...
          {isMulti && (
            <p className="text-xs text-fd-muted-foreground">
              Try <kbd className="font-mono">Space</kbd> /{" "}
              <kbd className="font-mono">Enter</kbd> to toggle,{" "}
              <kbd className="font-mono">Backspace</kbd> on the trigger to
              remove the last tag, <kbd className="font-mono">Escape</kbd> to
              close.
            </p>
          )}
        </>
